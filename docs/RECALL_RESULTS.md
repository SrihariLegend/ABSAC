...
| map-then-sum | `w03_sum_plus_one` | Still a recognition miss: the loop fact is a `sum`, but the accumulated value is a transformed element (`e + 1`), so no `SumReduction` truth is derived (D5 deliberately refuses transformed elements as raw sums). A real fix needs a mapped-sum concept with its own candidate that preserves the map. |
