...
| two-loop lowering | `w08_two_reductions`, v5/v4 `p12_two_loops`, v3 `p12_two_reductions` | **FAIL-CLOSED (2026-09-17), composition prototyped and reverted.** The lowerer can compose sequential self-latching loops (first loop's exit walk stops at the second's guard; shared exit/return once) and the result lowers+verifies+recognizes both reductions — but the SIR→C emitter models a SINGLE loop, so the emitted native code was wrong. The native differential caught it: w08 21/24, v3/v4 p12 21/24, v5 p12 24/24 mismatching cases with **0 rewrites** applied. The composition was reverted to the historical explicit refusal and the emitter now panics loudly on >1 Loop node. Final native sweep: 99 clean / 0 mismatched / 49 lower-refused. **Next step:** multi-loop emitter composition (emit each loop in order with its own dependency closure), then re-land the lowering behind it and re-run the native differential. Per-region outlining remains unusable as a composition basis (the w08 outline drops `add(%19,%6)`). |
