...
| early-exit gep | `x08_early_exit_write`, v6 `p09_first_set_const` (96), v7 `p09_first_set_early_return` (80) | **Lowering CLOSED for the canonical search CFG (2026-09-17)**: clang's early-return search is a header/latch/merge CFG (`for (i…) if (a[i]) return i;`). The lowerer now synthesizes the canonical found-flag SIR loop (`found' = found \| hit`, `index' = hit && !found ? i : sentinel`, termination `!found && i < BOUND`), the scans recognizer derives FirstOccurrence, and the existing bitscan recipes apply. Any non-matching CFG still falls back to the explicit refusal (fail closed). v6/v7 p09 now lower, promote to `[T; N]` and are **native-clean** (extents 96/80). The solver-backed rewrite is capped at 64-bit domains, so >64 extents lower+verify but stay unrewritten; a 48-element early-exit search lowers, is proven, rewrites to `ctz(pack)` and runs natively (5/0/47/48) — `emit_c_native` test. While landing this, the native differential also caught that the emitter had no `BoolAnd`/`BoolOr`/`BoolNot` arms (emitted `0`); they now emit `&&`/`||`/`!`. |
