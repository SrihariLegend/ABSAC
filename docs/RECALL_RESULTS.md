...
| early-exit gep | `x08_early_exit_write`, v6 `p09_first_set_const`, v7 `p09_first_set_early_return` | Still refused at lowering: `loop with a separate latch block (multi-block back-edge) not modeled`. Needs an early-exit control-flow model (SIR has no break/early-return inside loops). v7 re-confirmed on a fresh constant-extent kernel (safe refusal, 0 candidates). |
