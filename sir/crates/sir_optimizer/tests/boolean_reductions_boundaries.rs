...
fn build_non_boolean_array() -> sir_nodes::Function {
    ...
    let elem = b
        .array_access(board, i_initial, Type::u8(), Span::unknown())
        .unwrap();
...
fn build_mixed_reductions() -> sir_nodes::Function {
    ...
    let loop_node = b
        .r#loop(
            ...
            Type::Tuple {
                elements: vec![Type::i32(), Type::Bool, Type::u64()],
            },
            Span::unknown(),
        )
        .unwrap();
...
fn validate_boolean_boundaries() {
    ...
    for (name, func, expect_rewrite) in benchmarks {
        ...
        if !passed {
            all_passed = false;
            println!(
                "  => FAILED: Expected rewrite={}, but got rewrite={}",
                expect_rewrite, rewritten
            );
        }
    }
