...
pub struct SubgraphBuilder<'a> {
    arena: DetachedArena,
    next_local_id: u64,
    function: Option<&'a sir_nodes::Function>,
}
...
impl<'a> SubgraphBuilder<'a> {
    ...
    /// Create a new subgraph builder with context from the original function.
    pub fn with_function(function: &'a sir_nodes::Function) -> Self {
        Self {
            arena: DetachedArena::new(),
            next_local_id: 1_000_000_000,
    ...
    fn compute_effects(kind: &NodeKind) -> Effects {
        match kind {
            ...
            NodeKind::Loop { .. } => Effects::READ_MEMORY | Effects::WRITE_MEMORY,
            _ => Effects::empty(),
        }
    }

    pub fn get_type(&self, id: LocalNodeId) -> Option<Type> {
        self.arena.get(id).map(|n| n.ty.clone()).or_else(|| {
            self.function.and_then(|f| {
                f.get_node(sir_types::NodeId::new(id.as_u64()))
                    .map(|n| n.ty.clone())
            })
        })
    }
    ...
    pub fn finish(self, replacements: Vec<ReplacementValue>) -> ReplacementPatch {
        let roots: Vec<LocalNodeId> = self
            .arena
            .nodes()
            .map(|n| {
    ...
    /// LeadingZeros with an explicit result type (e.g. the loop index
    /// type when the bitscan result replaces a tuple element).
    pub fn leading_zeros_typed(
        &mut self,
        operand: LocalNodeId,
        ty: Type,
        span: Span,
    ...
    pub fn shl(&mut self, lhs: LocalNodeId, rhs: LocalNodeId, span: Span) -> LocalNodeId {
        let ty = self.get_type(lhs).unwrap_or(Type::i32());
        self.alloc_node(
            NodeKind::Shl {
                lhs: NodeId::new(lhs.as_u64()),
    ...
    pub fn pack(&mut self, array: LocalNodeId, width: usize, span: Span) -> LocalNodeId {
        self.alloc_node(
            NodeKind::Pack {
                array: NodeId::new(array.as_u64()),
            },
...
impl Default for SubgraphBuilder<'_> {
    fn default() -> Self {
        Self::new()
    }
}
