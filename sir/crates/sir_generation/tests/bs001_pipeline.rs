...
fn bs001_full_pipeline_produces_four_distinct_candidates() {
    ...
    // Four strategies apply to the index-scan count loop: Popcount,
    // BitIteration, PackedBitfield, MaskConstruction. (The second Popcount
    // strategy is scoped to BitsetIteration — Kernighan-style clear-lowest-
    // set-bit loops — and correctly does not apply here.)
    assert_eq!(
        total_candidates, 4,
